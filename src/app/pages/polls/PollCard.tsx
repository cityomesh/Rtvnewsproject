...
interface PollCardProps {
  poll: any;
  deletePoll?: (id: string) => Promise<void>;   // optional, we'll use our own
  showEdit?: boolean;
}

const PollCard: React.FC<PollCardProps> = ({ poll, showEdit = true }) => {
  const navigate = useNavigate();
  const id = poll.id;
  const [openModal, setOpenModal] = useState(false);
  const themeMode = useThemeMode();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Refresh the list – assume parent uses mutate or refetch
      mutate(`/polls/feed?page=0&size=10`);
    } catch (err) {
      console.error(err);
