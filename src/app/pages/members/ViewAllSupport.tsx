...
const ViewAllSupport = ()=>{
    ...
    const { data: staffsData, error: staffsError } = useSWR(
      `/members?page=${pageIndex}&size=9&memberType=SUPPORT_STAFF`,
      fetcher
    );
    ...
    useEffect(() => {
      ...
      if ( staffsError) {
        console.log(staffsError);
        toast.error("Failed to load Staff Details");
      }
    }, [staffsData, staffsError]);
    ...
    return (
        <>
          ...
          />}
                  ...
                  <div className="mb-8 d-flex">
                    ...
                    >
                        ...
                        Delete {selectedIds.size > 0 && (
                          <>
                            ({selectedIds.size} {selectedIds.size > 1 ? "Members" : "Member"})
                          </>
                        )}
                    </button>
                    <button 
            ...
            : <div className=''><NoData title={"Support Staff Details"} createUrl={"/supportstaff"}/></div>}
          ...
          <button
            disabled={ staffs?.length < 9 ? true : false }
            className='bg-light'
            onClick={() => setPageIndex(pageIndex + 1)}
            style={{
              marginLeft: "10px",
