...
const ReelCard: React.FC<ReelCardProps> = ({ reel, pageIndex, status }) => {
  ...
  const updateReelStatus = async (isApproved: boolean) => {
    try {
      const res = await client.post(
        `/reels/${reelId}/approve?isApproved=${isApproved}`,
  ...
  const formatToIST = (dateString: string) => {
    ...
    const [day, month, yearWithComma, time, ampm] = formatted
      .replace(",", "")
      .split(" ");
    ...
    <>
      ...
      {openRejectModal && (
        <RejectionModal
          ...
          onReject={() => updateReelStatus(false)}
          ...
          >
            ...
            >
              {status === "REVIEW_COMPLETE" ? (
                ...
                >
                  {reel.title?.length > 20
                    ? `${reel.title.slice(0, 20)}...`
            ...
            {status === "REVIEW_COMPLETE" && (
              ...
              >
                {reel?.uploadedBy === "admin" && (
                  <a
                    onClick={() => navigate(`/reels/create/${reelId}`)}
